  ...
  setup(props, context) {
    ...
    function loadAirQualityMetrics(stateChange) {
      ...
      for (var i = 0; i < airQualityLayers.length; i++) {
        if (airQualityLayers[i].id == store.state.exposure.prePolicyAirQualityId) {
         ...
         if (stateChange) {
          ...
          } else if (store.state.exposure.prePolicyAirQualityMetricId) {
            selectedItem.value = store.state.exposure.prePolicyAirQualityMetricId;
          }
    ...
    function loadMetricDetailsWhenReady(currentSelectedItem) {
      (async () => {
        ...
        while (!store.state.exposure.airQualityLayers)
          ...
          await new Promise((resolve) => setTimeout(resolve, 1000));
        console.log("airQualityLayers is defined");
        if (store.state.exposure.prePolicyAirQualityMetricId) {
          selectedItem.value = store.state.exposure.prePolicyAirQualityMetricId;
        }
    ...
    onMounted(() => {
      // var prePolicyAirQualityId = store.state.exposure.prePolicyAirQualityId;
      ...
      if (store.state.exposure.prePolicyAirQualityId) {
        ...
        selectedItem.value = store.state.exposure.prePolicyAirQualityMetricId;
        console.log("metricId: " + store.state.exposure.prePolicyAirQualityMetricId);
      }
    });
