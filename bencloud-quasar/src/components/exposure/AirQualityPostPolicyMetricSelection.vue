...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const scenarios = ref([]);
    const selectedScenario = ref(store.state.exposure.postPolicyAirQualitySelection);
    console.log(store.state.exposure.postPolicyAirQualitySelection);

    const options = ref([]);
    const rows = ref([]);

    watch(
      () => store.state.exposure.postPolicyAirQualityId,
    ...
    watch(
      ...
      (currentSelectedItem, prevSelectedItem) => {
        ...
        if(currentSelectedItem != prevSelectedItem) {
            store.commit(
              "exposure/updatePostPolicyAirQualitySelection",
              currentSelectedItem
            )
    ...
    function loadPostPolicyScenarios(stateChange) {
      scenarios.value = [];
      store.state.exposure.postPolicyAirQualityName.forEach(element => {
    ...
    function loadAirQualityMetrics(stateChange) {
      if (!store.state.exposure.airQualityLayers) {
      ...
      var airQualityLayers = JSON.parse(
        JSON.stringify(store.state.exposure.airQualityLayers.records)
      );
      console.log("-----");
      ...
      for (var i = 0; i < airQualityLayers.length; i++) {
        if (selectedScenario.value === airQualityLayers[i].name) {
          ...
          } else if (store.state.exposure.postPolicyAirQualityMetricId) {
            selectedItem.value = store.state.exposure.postPolicyAirQualityMetricId;
          }
    ...
    function loadMetricDetailsWhenReady(currentSelectedItem) {
      (async () => {
        ...
        while (!store.state.exposure.airQualityLayers)
          // define the condition as you like
          await new Promise((resolve) => setTimeout(resolve, 1000));
        console.log("airQualityLayers is defined");
        if (store.state.exposure.postPolicyAirQualityMetricId) {
          selectedItem.value = store.state.exposure.postPolicyAirQualityMetricId;
        }
    ...
    onMounted(() => {
      //var postPolicyAirQualityId = store.state.exposure.postPolicyAirQualityId;
      ...
      if (store.state.exposure.postPolicyAirQualityId) {
        ...
        selectedItem.value = store.state.exposure.postPolicyAirQualityMetricId;
        console.log("metricId: " + store.state.exposure.postPolicyAirQualityMetricId);
      }
    });
