  ...
  <div id="vue-example">
    ...
    <div id="drag-scope">
      <div class="column">
        ...
        <ToDoContainer class="regular green lighten-5" :data="todos.regular">
          <ToDoItem
            v-for="(items, index) in todos.regular"
            :key="items.item"
            :data="items.item"
            @remove="todos.regular.splice(index, 1)"
          ></ToDoItem>
        </ToDoContainer>
      </div>
      <div class="column">
        <div class="title">High Priority</div>
        <ToDoContainer class="priority red-4" :data="todos.priority">
          <ToDoItem
            v-for="(items, index) in todos.priority"
            :key="items.item"
            :data="items.item"
            @remove="todos.priority.splice(index, 1)"
          ></ToDoItem>
        </ToDoContainer>
      </div>
    </div>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    function handleDragEvent(event) {
      ...
      var element = this.getUnderlyingVm(newContainer);
      console.log(element)
