
export function healthEffectGroupId (state) {
    return state.healthEffectGroupId
}

export function healthEffectGroupName (state) {
  return state.healthEffectGroupName
}

export function healthEffectGroupAddedDate (state) {
  return state.healthEffectGroupAddedDate
}

export function valuationForceReloadValue (state) {
  return state.valuationForceReloadValue
}

export function healthEffectForceReloadValue (state) {
  return state.healthEffectForceReloadValue
}