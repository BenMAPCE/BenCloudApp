...
export default defineComponent({
    ...
    setup() {
        ...
        watch(
            ...
            (currentValue, newValue) => {
                if (!(prePolicyAirQualityIdHasValue(store) && 
                    ...
                    stepHasError.value && atStep.value.value == 2) {
                    $q.notify({
                        type: "negative",
        ...
        function differentGridDefinitionIdIndices() {
            ...
            if (store.state.exposure.postPolicyGridDefinitionId == null || store.state.exposure.prePolicyGridDefinitionId == null) {
                return indices;
            }

            for (let i = 0; i < store.state.exposure.postPolicyGridDefinitionId.length; i++) {
                if (store.state.exposure.postPolicyGridDefinitionId[i] !== store.state.exposure.prePolicyGridDefinitionId) {
                    indices.push(i);
                }
            }
        ...
        function validateGridDefinitionIds() {
            ...
            if (indices.length > 0) {
                ...
                const names = indices.map((i) => "\"" + store.state.exposure.postPolicyAirQualityName[i].name +
                     "\" uses \"" + store.state.exposure.postPolicyGridDefinitionName[i] + "\"").join(", ");
                var mismatchMessage = "The pre-policy and post-policy air quality surfaces must use "+
                    ...
                    store.state.exposure.prePolicyGridDefinitionName + "\" grid however the" + (plural?"es":"") +
